...
public class Event3 : MonoBehaviour
{
    ...
    {
        array = GameObject.FindGameObjectsWithTag("DMG3");
        array2 = GameObject.FindGameObjectsWithTag("DMG3-1");
        Debug.Log("DMG3의 오브젝트 개수는 " + array.Length + "이다.");



        if (manager.sit3 == false) // 상황3이 한번도 실행 안 된 상태만 아래의 스크립트 실행됨
        {
            manager.sit3 = true; // 상황3이 더 이상 실행되지 않도록 true로 잠궈버리기
            manager.check2 = true;

            t.color = Color.red;
            t.text = "왼편의 책장을 주의하십시오!";
